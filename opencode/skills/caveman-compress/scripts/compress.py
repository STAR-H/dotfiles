...
def model_name_for_http() -> str:
    model = os.environ.get("CAVEMAN_MODEL") or os.environ.get("OPENCODE_MODEL") or "gpt-5.5"
    return model.split("/", 1)[1] if "/" in model else model


def call_opencode_compatible(prompt: str) -> str:
    """Send a prompt to the OpenAI-compatible provider used by opencode."""
    base_url = os.environ.get("OPENAI_BASE_URL")
    api_key = os.environ.get("OPENAI_API_KEY")
    if not base_url or not api_key:
        raise RuntimeError(
            "opencode-compatible backend requires OPENAI_BASE_URL and OPENAI_API_KEY"
        )

    url = base_url.rstrip("/") + "/chat/completions"
    body = {
        "model": model_name_for_http(),
    ...
    try:
        ...
        raise RuntimeError(f"opencode-compatible call failed ({e.code}):\n{detail}")

    try:
        content = payload["choices"][0]["message"]["content"]
    except (KeyError, IndexError, TypeError) as e:
        raise RuntimeError(f"opencode-compatible response missing message content: {payload}") from e
...
def call_llm(prompt: str) -> str:
    backend = os.environ.get("CAVEMAN_BACKEND", "auto").lower()
    if backend == "opencode":
        return call_opencode_compatible(prompt)
    if backend in ("claude", "claude-cli", "anthropic"):
        return call_claude(prompt)
    if backend != "auto":
        raise RuntimeError(f"Unknown CAVEMAN_BACKEND: {backend}")
